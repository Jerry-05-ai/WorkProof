...
async function getUserByEmail(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
...
async function getUserById(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function getCompanyById(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function getCompanyByName(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function getCompanyByEmail(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function getEmployeeById(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function getEmployeeByEmail(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function listEmployeesByCompany(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function searchEmployees(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function getSkillByName(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function listSkillsByEmployee(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function getEmployeeSkillByPair(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function listProjectsByEmployee(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function listProjectsByCompany(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function listBehaviorRatings(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function latestBehaviorRatings(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function listAchievements(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function listPerformanceReviews(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function getLatestPerformanceReview(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function listMonthlyReports(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function getMonthlyReportByKey(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function getPrivacyByEmployee(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function getEmployeeEmploymentStatus(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function getPublicProfileByEmployee(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function getPublicProfileBySlug(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function checkSlugExists(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function listPublicProfiles(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function getVerificationCorrectionById(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function listVerificationCorrectionsByEmployee(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function listVerificationCorrectionsByCompany(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function getJobOpportunityById(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function listJobOpportunitiesByRecruiter(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function listJobOpportunitiesByEmployee(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function countJobOpportunitiesByStatus(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function listEmploymentLinksByPerson(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function getEmploymentLinkByEmployee(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function getCurrentEmploymentLink(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function listNotificationsByUser(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function listUnreadNotificationsByUser(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function getNotificationById(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function countUnreadNotifications(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function listAuditLogs(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function countAuditLogs(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function listInternalProjectsByCompany(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function getInternalProjectById(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function getInvitationByToken(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function checkPendingInvitation(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function getSavedCandidate(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function listSavedCandidatesByRecruiter(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function companyEmployeeStats(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function companySkillDistribution(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function adminDashboardStats(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function findPlatformAdmins(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function adminListCompanies(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function allOpportunityStatuses(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function topSkills(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function createUser(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function updateUserLastLogin(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function setUserCompanyAndEmployee(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function createCompany(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function setCompanyAdmin(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function updateCompanyStatus(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function createCompanyMembership(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function createEmployee(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function updateEmployee(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function softDeleteEmployee(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function verifyEmployeeRecord(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function endEmployeeEmployment(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function createSkill(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function addEmployeeSkill(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function updateEmployeeSkill(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function createProject(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function updateProject(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function createBehaviorRating(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function createAchievement(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function verifyAchievement(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function createPerformanceReview(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function createMonthlyReport(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function updateMonthlyReport(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function createPrivacySettings(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function updatePrivacySettings(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function transferPrivacyOwnership(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function markPrivacyPublished(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function createPublicProfile(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function updatePublicProfileVisibility(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function incrementPublicProfileViews(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function createVerificationCorrection(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function updateVerificationCorrectionStatus(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function createJobOpportunity(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function updateJobOpportunityStatus(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function createEmploymentLink(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function markEmploymentLinkLeft(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function attachUserToEmploymentLinks(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function createNotification(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function markNotificationRead(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function markAllNotificationsRead(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function createAuditLog(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function createInternalProject(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function updateInternalProject(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function softDeleteInternalProject(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function createInvitation(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function markInvitationAccepted(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function saveCandidate(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function updateSavedCandidateNotes(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
...
async function deleteSavedCandidate(dcOrVars, vars) {
  let dc = dcOrVars;
  let variables = vars;
  if (!dc || typeof dc.executeGraphql !== 'function') {
    // First arg was actually the variables object; resolve default DataConnect instance.
